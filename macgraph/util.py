...
"""
def pad_to_table_len(tensor, table_to_mimic=None, seq_len=None, name=None):
	if seq_len is not None:
		delta = seq_len - tf.shape(tensor)[1]
	elif table_to_mimic is not None:
		delta = tf.shape(table_to_mimic)[1] - tf.shape(tensor)[1]
	else:
		raise Exception("Must have argument table_to_mimic or seq_len")

	tensor = tf.pad(tensor, [ [0,0], [0,delta], [0,0] ], name=name) # zero pad out

	if seq_len is not None:
		tensor.set_shape([None, seq_len, None])
